...
def read_file(name):
    ...
    tasks = (delayed(all_features)(seg) for seg in file_segments_transposed))
    inputs = np.array(Parallel(n_jobs=4)(tasks)
    return inputs
...
out_dir = os.path.join(data_dir, 'interim', 'complete', 'X')
in_dir = os.path.join(data_dir, 'external', 'recordings')

eeg_files = list_absolute(in_dir)
